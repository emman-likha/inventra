      ...
      <motion.div variants={fadeUp}>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-xs text-foreground/30 mt-1 mb-6">Get started by adding your first department or importing from a file.</p>
          ...
          <div className="border border-foreground/[0.08] rounded-2xl overflow-x-hidden overflow-y-auto max-h-[520px] scrollbar-hidden">
            <table className="w-full">
              ...
              <tbody>
                {filtered.map((dept) => {
                  const memberCount = dept.member_count;
                  return (
                    <tr
                      key={dept.id}
                      onClick={() => router.push(`/dashboard/departments/${dept.id}`)}
                      className="border-b border-foreground/[0.06] last:border-0 hover:bg-foreground/[0.03] transition-colors cursor-pointer"
                    >
