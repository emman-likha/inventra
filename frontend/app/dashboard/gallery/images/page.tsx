...
export default function ImagesGalleryPage() {
  ...
  const { data: files = [], isLoading } = useQuery<StorageFile[]>({
    ...
    queryFn: async () => {
      if (!companyId) return [];
      // List all files under the company folder (includes all users' files)
  ...
  function getPublicUrl(name: string) {
    const { data } = supabase.storage.from(BUCKET).getPublicUrl(`${companyId}/${name}`);
  ...
  return (
    <div className={hasFiles ? "" : "flex flex-col h-[calc(100vh-5rem)] lg:h-[calc(100vh-5rem)]"}>
      ...
      {isLoading ? (
        ...
        /* Empty state: full-viewport drop zone */
        <div
        ...
        /* Has files: search bar with upload button + image grid */
        <>
