...
import { importInventories, type InventoryInput } from "@/lib/api";
...
export function ImportInventoryModal({ open, onClose }: ImportInventoryModalProps) {
  ...
  const [preview, setPreview] = useState<InventoryInput[]>([]);
  const [parseError, setParseError] = useState("");
  const [isDragging, setIsDragging] = useState(false);

  const mutation = useMutation({
    mutationFn: async (inventories: InventoryInput[]) => {
      await importInventories(inventories);
    },
    onSuccess: () => {
  ...
  function validateRows(rows: Record<string, string>[], headers: string[]): { parsed: InventoryInput[]; error?: string } {
    ...
    if (missing.length > 0) {
      return { parsed: [], error: `Missing required column: name/item.\nDetected columns: ${headers.join(", ")}` };
    }

    const parsed: InventoryInput[] = [];
    ...
    rows.forEach((row) => {
      ...
      const qty = normalized["quantity"] ? parseInt(normalized["quantity"]) : 0;
      const minQty = normalized["min_quantity"] ? parseInt(normalized["min_quantity"]) : 0;
      const cost = normalized["cost_per_unit"] ? parseFloat(normalized["cost_per_unit"]) : undefined;
      ...
      parsed.push({
        ...
        cost_per_unit: cost !== undefined && !isNaN(cost) ? cost : null,
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div
              className="bg-background border border-foreground/[0.08] rounded-2xl w-full max-w-xl p-6 pointer-events-auto shadow-xl max-h-[85vh] overflow-y-auto"
                ...
                <>
                  ...
                  <div className="mt-4 bg-foreground/[0.02] border border-foreground/[0.06] rounded-xl p-4">
                    ...
                    <p className="text-xs text-foreground/35 mb-3">
                      Required: <span className="text-foreground/55 font-medium">name</span>.
                      Optional: <span className="text-foreground/55 font-medium">category, quantity, min_quantity, unit, cost_per_unit, location, description</span>
                    </p>
                    <div className="bg-foreground/[0.03] rounded-lg p-3 font-mono text-[11px] text-foreground/40 leading-relaxed overflow-x-auto">
                      name, category, quantity, unit, cost_per_unit, location<br />
                      DDR4 RAM 16GB, Spare Parts, 50, pcs, 45.99, Warehouse A<br />
                      USB-C Cable, Accessories, 200, pcs, 8.50, Office B<br />
                      Printer Ink Black, Consumables, 30, pcs, 24.00, Storage
                    </div>
                  </div>
                </>
              ) : preview.length > 0 ? (
                <>
                  <div className="flex items-center gap-3 mb-4 bg-foreground/[0.03] border border-foreground/[0.08] rounded-xl px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-foreground/35">{preview.length} {preview.length === 1 ? "item" : "items"} ready to import</p>
                    </div>
                    <button
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-foreground/[0.08]">
                          ...
                          <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-foreground/40 uppercase tracking-wider">Qty</th>
                        </tr>
                      </thead>
                      <tbody>
                        {preview.slice(0, 10).map((inv, i) => (
                          <tr key={i} className="border-b border-foreground/[0.04] last:border-0">
                            <td className="px-4 py-2 text-xs text-foreground">{inv.name}</td>
                            <td className="px-4 py-2 text-xs text-foreground/50">{inv.category || "—"}</td>
                            <td className="px-4 py-2 text-xs text-foreground/50">{inv.quantity ?? 0} {inv.unit || "pcs"}</td>
                          </tr>
                        ))}
                        {preview.length > 10 && (
                          <tr>
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      {mutation.isPending ? "Importing..." : `Import ${preview.length} Items`}
                    </button>
                  </div>
                </>
              ) : null}
