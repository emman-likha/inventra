...
// Maps common status text to our valid status values
const STATUS_ALIASES: Record<string, string> = {
  ...
  "in-use": "checked_out",
  "active": "checked_out",
  "assigned": "checked_out",
  "deployed": "checked_out",
  "charging": "checked_out",
...
export function ImportAssetModal({ open, onClose }: ImportAssetModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (assets: ParsedAsset[]) => {
  ...
  function resolveColumnName(header: string): string | null {
    ...
    // Partial match: check if any alias is contained in the header
  ...
  function validateRows(rows: Record<string, string>[], headers: string[]): { parsed: ParsedAsset[]; error?: string } {
    // Build a mapping from original headers to standard field names
    ...
    if (missing.length > 0) {
      return { parsed: [], error: `Missing required columns: ${missing.join(", ")}. Required: name, category.\nDetected columns: ${headers.join(", ")}` };
    }
    ...
    rows.forEach((row, i) => {
      ...
      if (!name || !category) {
        errors.push(`Row ${i + 2}: name and category are required.`);
        return;
      }

      // Resolve status via aliases
      const rawStatus = normalized["status"]?.toLowerCase().trim() || "";
      const status = rawStatus ? (STATUS_ALIASES[rawStatus] || rawStatus) : "available";
      if (!VALID_STATUSES.includes(status)) {
        errors.push(`Row ${i + 2}: unrecognized status "${normalized["status"]}".`);
        return;
      }

      const valueStr = normalized["value"];
      ...
      if (valueStr) {
        ...
        if (isNaN(num)) {
          errors.push(`Row ${i + 2}: invalid value "${valueStr}".`);
          return;
        }
        value = num;
      }
    ...
    if (errors.length > 0) {
      return { parsed: [], error: errors.slice(0, 3).join("\n") + (errors.length > 3 ? `\n...and ${errors.length - 3} more errors.` : "") };
    }
                ...
                <>
                  ...
                  <div className="mt-4 bg-foreground/[0.02] border border-foreground/[0.06] rounded-xl p-4">
                    ...
                    <p className="text-xs text-foreground/35 mb-2">
                      Required columns: <span className="text-foreground/55 font-medium">name</span>, <span className="text-foreground/55 font-medium">category</span>
                    </p>
                    <p className="text-xs text-foreground/35 mb-3">
                      Optional: <span className="text-foreground/55 font-medium">location</span>, <span className="text-foreground/55 font-medium">status</span>, <span className="text-foreground/55 font-medium">value</span>
                    </p>
                    <div className="bg-foreground/[0.03] rounded-lg p-3 font-mono text-[11px] text-foreground/40 leading-relaxed overflow-x-auto">
                      name,category,location,status,value<br />
                      MacBook Pro 16&quot;,Laptops,Office A,available,2499<br />
                      Dell Monitor,Monitors,Floor 3,checked_out,450
                    </div>
                  </div>
                </>
              ) : preview.length > 0 ? (
                <>
                  ...
                  <div className="flex items-center gap-3 mb-4 bg-foreground/[0.03] border border-foreground/[0.08] rounded-xl px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-foreground/35">{preview.length} {preview.length === 1 ? "asset" : "assets"} ready to import</p>
                    </div>
                    <button
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-foreground/[0.08]">
                          ...
                          <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-foreground/40 uppercase tracking-wider">Category</th>
                          <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-foreground/40 uppercase tracking-wider">Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        {preview.slice(0, 10).map((a, i) => (
                          <tr key={i} className="border-b border-foreground/[0.04] last:border-0">
                            <td className="px-4 py-2 text-xs text-foreground">{a.name}</td>
                            <td className="px-4 py-2 text-xs text-foreground/55">{a.category}</td>
                            <td className="px-4 py-2 text-xs text-foreground/55 capitalize">{a.status.replace("_", " ")}</td>
                          </tr>
                        ))}
                        {preview.length > 10 && (
                          <tr>
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      {mutation.isPending ? "Importing..." : `Import ${preview.length} Assets`}
                    </button>
                  </div>
                </>
              ) : null}
